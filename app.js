...
function _fbLoad(){
  db.doc(DB_DOC).get().then(function(doc){
    ...
    if(doc.exists){
      ...
      renderAll();
      updateSyncIndicator();
      migrarDatos(); // migración silenciosa al cargar
...
function renderResumen(){
  ...
  // ── Ganancia Bruta y Neta del mes ──
  // Bruta = Σ (precioVenta − precioCosto) × cant de cada item vendido con producto vinculado
  //          + monto completo de ventas manuales sin producto (sin costo registrado)
  // Neta  = Bruta − gastos operativos del mes (excluye pedidos de compra)
  var mesActual=dateKey(currentDate).substring(0,7);
  var ganancia=0;    // Ganancia Bruta acumulada del mes
  var gastosTotal=0; // Gastos operativos del mes (no pedidos de compra)
  Object.keys(data).forEach(function(k){
    if(k.substring(0,7)!==mesActual) return;
    var d=data[k];
    (d.ventas||[]).forEach(function(v){
      var items=v.items&&v.items.length ? v.items : [{desc:v.desc,cant:v.cant,monto:v.monto,productoId:v.productoId}];
      items.forEach(function(it){
        if(it.productoId){
          var prod=getProducto(it.productoId);
          if(prod && prod.precioCosto>0 && prod.precioVenta>0){
            // Margen real: (precioVenta - precioCosto) * cant vendida
            ganancia += (prod.precioVenta - prod.precioCosto) * (it.cant||1);
          } else {
            // Sin precio costo registrado: se suma el monto completo como ganancia
            ganancia += it.monto;
          }
        } else {
          // Venta manual sin producto: monto completo es ganancia
          ganancia += it.monto;
        }
      });
    });
    // Solo gastos operativos (excluir los que son de pedidos de compra)
  ...
  if(ganSub) ganSub.textContent='Margen venta−costo — '+mesLabel;
  if(ganNetaSub) ganNetaSub.textContent='Bruta − gastos operativos';
...
function renderRankingVentas(){
  ...
  var html='<div style="font-size:11px;color:var(--text3);margin-bottom:8px">Acumulado de todos los días registrados</div>';
  lista.forEach(function(p,i){
    ...
    html+='<div class="card" style="padding:12px 16px;margin-bottom:8px">'+
      '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'+
        ...
        '<div style="font-family:\'DM Mono\',monospace;font-size:13px;color:var(--green);font-weight:600">'+fmt(p.monto)+'</div>'+
      '</div>'+
      '<div style="display:flex;align-items:center;gap:8px">'+
        ...
        '<div style="font-size:11px;color:var(--text3);white-space:nowrap">'+p.cant.toFixed(1)+' uds</div>'+
      '</div>'+
    '</div>';
  });
  el.innerHTML=html;
...
function verDetalle(tipo, id){
  ...
  if(!isV&&(it.categoria||it.pedidoId)){
    var cat=it.pedidoId?'pedido':(it.categoria||'otro');
    html+='<div style="display:flex;justify-content:space-between;margin-bottom:8px"><span style="font-size:12px;color:var(--text3)">Categoría</span><strong>'+(CATS2[cat]||cat)+'</strong></div>';
  }
  ...
  if(isV&&it.productoNombre) html+='<div style="display:flex;justify-content:space-between;margin-bottom:8px"><span style="font-size:12px;color:var(--text3)">Producto</span><strong>'+esc(it.productoNombre)+'</strong></div>';
  if(it.pedidoId) html+='<div style="display:flex;justify-content:space-between;margin-bottom:8px"><span style="font-size:12px;color:var(--text3)">Origen</span><strong>📦 Pedido completado</strong></div>';
  if(it.items&&it.items.length){
    html+='<div style="border-top:1px solid var(--border);margin:10px 0 8px;padding-top:8px;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--text3)">Productos incluidos</div>';
    it.items.forEach(function(x){
      html+='<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--border)">'+
        ...
        '<div style="font-family:\'DM Mono\',monospace;font-size:13px;font-weight:600;color:'+(isV?'var(--green)':'var(--red)')+'">'+fmt(x.monto)+'</div>'+
      '</div>';
    });
  }
  var color=isV?'var(--green)':'var(--red)';
...
function migrarDatos(){
  ...
  Object.keys(data).forEach(function(k){
    var d=data[k];
    (d.gastos||[]).forEach(function(g){
      // Marcar gastos de pedidos que no tienen pedidoId
      if(!g.pedidoId && g.desc && g.desc.indexOf('Pedido:')===0){
        g.pedidoId='legacy'; // marcar como legacy para excluirlos de ganancia neta
        cambios++;
      }
      // Asignar categoría por defecto a gastos sin categoría
      if(!g.categoria && !g.pedidoId){
        var d2=g.desc?g.desc.toLowerCase():'';
        if(d2.indexOf('pasaje')!==-1||d2.indexOf('transporte')!==-1||d2.indexOf('bus')!==-1) g.categoria='pasaje';
        else if(d2.indexOf('aliment')!==-1||d2.indexOf('comida')!==-1||d2.indexOf('almuerzo')!==-1||d2.indexOf('desayuno')!==-1) g.categoria='alimentacion';
        else if(d2.indexOf('alcald')!==-1||d2.indexOf('impuesto')!==-1||d2.indexOf('municipal')!==-1) g.categoria='alcaldia';
        else if(d2.indexOf('luz')!==-1||d2.indexOf('electric')!==-1||d2.indexOf('energía')!==-1||d2.indexOf('energ')!==-1) g.categoria='electricidad';
  ...
  if(cambios>0){
    saveData();
    _fbWrite();
    toast('✓ Migración completa: '+cambios+' registros actualizados');
  } else {
    toast('Sin cambios — datos ya están al día');
  }
  renderAll();
}
...
function renderPedidoItems(){
  ...
  if(!pedidoItems.length){
    el.innerHTML='<div style="font-size:12px;color:var(--text3);text-align:center;padding:8px;background:var(--surface2);border-radius:var(--radius-sm)">Selecciona un producto y presiona <strong>+ Agregar</strong>, o registra el pedido manualmente abajo.</div>';
    return;
  }
  ...
  pedidoItems.forEach(function(it,i){
    html+='<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--surface2);border-radius:var(--radius-sm);border:1px solid var(--border)">'+
      '<div style="flex:1;min-width:0">'+
        '<div style="font-size:13px;font-weight:500">'+esc(it.desc)+'</div>'+
        '<div style="font-size:11px;color:var(--text3)">Cant: '+it.cant+' '+it.unidad+(it.productoNombre?' · '+esc(it.productoNombre):'')+'</div>'+
      '</div>'+
      '<div style="font-family:DM Mono,monospace;font-size:13px;color:var(--orange);font-weight:500">'+fmt(it.monto)+'</div>'+
  ...
  html+='<div style="text-align:right;font-size:13px;font-weight:700;color:var(--orange);padding:4px 2px">Total: '+fmt(total)+'</div>';
  el.innerHTML=html;
}
function toast(msg){
